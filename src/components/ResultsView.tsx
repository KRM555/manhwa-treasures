                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pl-1 pr-1">
                  {filteredBubbles?.map((bubble: any, bIdx: number) => (
                    ...
                    >
                      ...
                      <div
                        className="text-xs text-slate-400 font-mono bg-slate-900/80 p-2.5 rounded-lg border border-slate-800/60 leading-relaxed text-left"
                        dir="ltr"
                      >
                        {bubble.originalText}
                      </div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          onUpdateBubble(currentPage.id, bubble.id, e.target.value, bubble.category)
                        }
                        rows={3}
                        dir="auto"
            ...
            <div className="flex-1 bg-slate-900 border border-slate-800 rounded-2xl p-6 flex gap-8 h-full min-h-0 overflow-y-auto custom-scrollbar shadow-xl">
              <div className="flex-1 flex justify-center items-start bg-slate-950 rounded-xl p-4 border border-slate-800">
                <img
                  src={currentPage.imageUrl}
                  alt="Preview"
                  className="w-auto max-w-full object-contain rounded-lg shadow-2xl"
                />
              </div>
              <div className="w-[500px] space-y-4">
                ...
                <div className="space-y-3 font-mono text-xs">
                  {currentPage.bubbles?.map((b: any, idx: number) => (
                    <div
                      key={b.id || idx}
                      className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1 shadow-inner"
                    >
                      <div className="text-[10px] text-orange-400/80 font-bold">
                        Bubble #{idx + 1} ({b.category || "dialogue"})
                      </div>
                      <div
                        className="text-sm font-bold text-slate-100 leading-relaxed select-all"
                        dir="auto"
                      >
                        {formatBubbleText(b)}
                      </div>
